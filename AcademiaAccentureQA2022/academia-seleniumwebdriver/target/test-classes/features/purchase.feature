@purchase


Feature: E-commerce shopping

	Background: Abrir o site
		Given usuario acessa o site "http://automationpractice.com/index.php"

	  
	
	Scenario Outline: Logar no Site
		#When preenche as "<emailLogin>", "<passwdLogin>" credenciais validas
		And escolhe o departamento feminino
	  And escolhe um produto disponivel
		And coloca no carrinho de compras
		And confirma o checkout
		#And adciona mais um produto 
		And confirma o segundo checkout
		And deixei um "<leiam>" recado para o pessoal da academia qa2022 
		And confirma o terceiro checkout 
		And confirma os termos de servico
		And confirma o quarto checkout
		And seleciona pagamento por cheque 
		Then seu pedido foi realizado com sucesso
		
		Examples:
		|emailLogin                  |passwdLogin |leiam                                | 
		|moura.camilaluiza@gmail.com |Teste@123   |Thanks, ladies and gentlemen, you've been great. |
